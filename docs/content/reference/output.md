...
describes what users get. Pick a format with `-o`, or let weibo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
weibo <command> -o table   # aligned columns for reading
weibo <command> -o jsonl   # one JSON object per line, for piping
weibo <command> -o json    # a single JSON array
weibo <command> -o csv     # spreadsheet friendly
weibo <command> -o tsv     # tab-separated
weibo <command> -o url     # just the URL column
weibo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
weibo <command> --fields id,title,url
```
...
```bash
weibo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
weibo <command>            # a table, because this is a terminal
weibo <command> | wc -l    # JSONL, because this is a pipe
```
